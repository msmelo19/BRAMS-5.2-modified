...
for i in $(seq 1 3); do
	if [ -z ${SLURM_RESTART_COUNT} ] || [ ${SLURM_RESTART_COUNT} -eq 0 ]; then
	  ...
	  echo "HISTORY"
		date > ${RESULT_DIR}/date-his-${SLURM_RESTART_COUNT}-${SLURM_JOB_ID}.txt
	  ./create-ramsin-history-mode.sh ${RAMSIN_INITIAL_PATH}
	  mpirun --map-by ppr:${SLURM_NTASKS_PER_NODE}:node ${BRAMS_BIN} -f ${RAMSIN_INITIAL_PATH}-history 2>&1 | tee ${RESULT_DIR}/brams-history-${SLURM_NTASKS}-${SLURM_JOB_ID}-${i}-${SLURM_RESTART_COUNT}.out
	fi
done

date > ${RESULT_DIR}/date-fim-${SLURM_JOB_ID}.txt
