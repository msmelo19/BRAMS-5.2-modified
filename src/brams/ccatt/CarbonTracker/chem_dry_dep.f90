...
!DSM    CALL dry_dep_gases(m1,m2,m3,nspecies_chem,npatch,ia,iz,ja,jz  &
!DSM         ,v_dep_gas,r_aer  		           &
!DSM         ,prss,temps,dens,vels,rvs,rcp,Zi		   &
!DSM         ,ustar,tstar,patch_area,veg,Z0m		   &
!DSM         ,rshort,rtgt,dzt,check_rain,rmol,rhchem &
!DSM         ,O3,spc_alloc_chem,transport &
!DSM         ,on,dvj,hstar,ak0,dak,dhr,f0,imonth1,idate1,iyear1)
...
!DSM  SUBROUTINE dry_dep_gases(m1,m2,m3,nspecies_chem,npatch,ia,iz,ja,jz         &
!DSM                          ,V_dep,r_aer,prss,temps,dens,vels,rvs,rcp,Zi       &
!DSM                          ,ustar,tstar,patch_area,veg,Z0m,rshort,rtgt,dzt    &
!DSM                          ,check_rain,rmol,rhchem,O3,spc_alloc_chem,transport&
!DSM                          ,on,dvj,hstar,ak0,dak,dhr,f0,imonth1,idate1,iyear1)
  SUBROUTINE dry_dep_gases(m1,m2,m3,nspecies_chem,npatch,ia,iz,ja,jz         &
                          ...
                          ,check_rain,rmol,rhchem,spc_alloc_chem,transport&
                          ,on,dvj,hstar,ak0,dak,dhr,f0,imonth1,idate1,iyear1)
       ...
       DO i = ia,iz
          ...
          DO ipatch = 1,npatch
             IF (patch_area(i,j,ipatch) .GE. .009) THEN 
                ...
                !DSM CALL rc(rcx,temps(i,j),rshort(i,j),rhchem(i,j),iveg,iseason   &
                !DSM      ,nspecies_chem,wetflag,rainflag,highnh3,spc_alloc_chem   &
                !DSM      ,hstar,ak0,dak,dhr,f0,transport,O3,on)

                CALL rc(rcx,temps(i,j),rshort(i,j),rhchem(i,j),iveg,iseason   &
                     ,nspecies_chem,wetflag,rainflag,highnh3,spc_alloc_chem   &
                     ,hstar,ak0,dak,dhr,f0,transport,on)
...
!!$	    endif
  ...
  ! **************************  SUBROUTINE RC  ***************************
  ! **********************************************************************
!DSM   SUBROUTINE rc(rcx,t,rad,rh,iland,iseason,nspecies_chem,        &
!DSM                 wetflag,rainflag,highnh3,spc_alloc_chem,hstar,   &
!DSM                 ak0,dak,dhr,f0,transport,O3,on )
  SUBROUTINE rc(rcx,t,rad,rh,iland,iseason,nspecies_chem,        &
                wetflag,rainflag,highnh3,spc_alloc_chem,hstar,   &
                ak0,dak,dhr,f0,transport,on )
...
!DSM    IF( spc_alloc_chem(transport,O3) == ON) THEN

       !--------------------------------- O3 
       !        SPECIAL TREATMENT FOR OZONE
       !srf       hstary(O3) = hstar(O3)*exp(dhr(O3)*(1./t-1./298.))
!DSM       hstary(O3) = hstar(O3)*EXP(dhr(O3)*(temp_i))
!DSM       rmx = 1./(hstary(O3)/3000.+100.*f0(O3))
!DSM       rsmx = rs*dratio(O3) + rmx

!DSM !--(DMK-CCATT-INI)----------------------------------------------------------
!DSM        rlux = rlu(ixxxlu(iland),iseason)/(1.E-5*hstary(O3)+f0(O3)) + resice
!DSM        rclx = rclo(ixxxlu(iland),iseason) + resice
!DSM        rgsx = rgso(ixxxlu(iland),iseason) + resice
!DSM !--(DMK-CCATT-OLD)----------------------------------------------------------
!DSM !       rlux = rlu(iland,iseason)/(1.E-5*hstary(O3)+f0(O3)) + resice
!DSM !       rclx = rclo(iland,iseason) + resice
!DSM !       rgsx = rgso(iland,iseason) + resice
!DSM !--(DMK-CCATT-FIM)----------------------------------------------------------

!DSM        IF (wetflag) rlux = rluo1
!DSM        IF (rainflag) rlux = rluo2

!DSM !--(DMK-CCATT-INI)----------------------------------------------------------
!DSM        rcx(O3) = 1./(1./rsmx+1./rlux+1./(rdc+rclx)+1./(rac(ixxxlu(iland), &
!DSM             iseason)+rgsx))
!DSM !--(DMK-CCATT-OLD)----------------------------------------------------------
!DSM !       rcx(O3) = 1./(1./rsmx+1./rlux+1./(rdc+rclx)+1./(rac(iland, &
!DSM !            iseason)+rgsx))
!DSM !--(DMK-CCATT-FIM)----------------------------------------------------------

!DSM       rcx(O3)=MAX(rcx(O3),1.) !- old: IF (rcx(O3)<1.) rcx(O3) = 1.
!DSM    ENDIF
