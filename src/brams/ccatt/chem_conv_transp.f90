       ...
       do ispc=1,nspecies
        sc_up(ispc,k)=  (              zu(k-1)*sc_up(ispc,k-1)  - &
	                  0.5*up_massdetr(k-1)*sc_up(ispc,k-1)  + &
                         ...
                         (zu(k-1)-0.5*up_massdetr(k-1)+up_massentr(k-1))
       enddo
...
!- - - - - - - model level 1 - up_massentr, up_massdetr , se
  ...
  do k=2,ktop
     ...
     !-- special levels
     if(k.eq.jmin .and. iinqparm .ne. 6)   entdoj = edt*zd(k)
     if(k.eq.ktop) then
        detupk  = zu(ktop)
        subin   = 0.
        detdo   = 0.
  ...
  do k=2,n1-1
     ...
     do ispc = nchem_a,nchem_z
       ...
       if(se(ispc,k) < -0.5) then
          write(mynum,*),'CHEM i, j,mynum,k,ispc',i, j,mynum,k,ispc
          write(mynum,*),se(ispc,k),stcum1d(ispc,k)
          flush(mynum)
         ! stop 444
       endif  
...
!     enddo
     ...
     ! if(ispc==34) print*,'SE=',se(ispc,k),se(ispc,k)+dt*stcum1d(ispc,k)  
       ...
       se    (ispc,k) =  se    (ispc,k)  + dt *  stcum1d(ispc,k)           ! mixing ratio on Z levels
        ...
        if(se(ispc,k) < -0.5) then
          write(mynum,*),'AER i, j,mynum,k,ispc',i, j,mynum,k,ispc
          write(mynum,*),se(ispc,k),stcum1d(ispc,k)
          flush(mynum)
         ! stop 500
       endif  
