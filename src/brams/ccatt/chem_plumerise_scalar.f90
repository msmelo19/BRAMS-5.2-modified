...
!!$    REAL,                   INTENT(IN)    :: zt_rams (nzpmax,mxp,myp)
!!$    REAL,                   INTENT(IN)    :: zm_rams (nzpmax,mxp,myp)
!!$    REAL,                   INTENT(IN)    :: dzt_rams(nzpmax,mxp,myp)
...
!!$    TYPE (aer1_vars),       INTENT(INOUT) :: aer1_g(nmodes,aer_nspecies)
    ...
    ! mem_plume_chem1
!!$    TYPE (plume_mean_vars), INTENT(IN)    :: plume_mean_g(nveg_agreg)
...
!!$    REAL,                  INTENT(IN)     :: up   (m1,m2,m3)!srf-AWE
!!$    REAL,                  INTENT(IN)     :: vp   (m1,m2,m3)!srf-AWE
!!$    REAL,                  INTENT(IN)     :: rtgt (m2,m3)
!!$    INTEGER,               INTENT(IN)     :: lpw  (m2,m3)
!!$    REAL,                  INTENT(IN)     :: zt_rams(m1,m2,m3)
!!$    REAL,                  INTENT(IN)     :: zm_rams(m1,m2,m3)
    REAL,                   INTENT(IN)    :: theta(:,:,:)
    REAL,                   INTENT(IN)    :: pp   (:,:,:)
    REAL,                   INTENT(IN)    :: pi0  (:,:,:)
    REAL,                   INTENT(IN)    :: rv   (:,:,:)
    ...
    INTEGER,               INTENT(IN)     :: nzpmax
    ! mem_grid
!!$    REAL,                  INTENT(IN)     :: dzt_rams(nzpmax,m2,m3)
    REAL,                  INTENT(IN)     :: dzt_rams(:)
    ! chem1_list
    INTEGER,               INTENT(IN)     :: chem_nspecies
!!$    INTEGER,               INTENT(IN)     :: spc_chem_alloc(6,chem_nspecies)
...
!!$    TYPE (plume_mean_vars), INTENT(IN)    :: plume_mean_g(nveg_agreg)
    ...
    INTEGER            :: i, j, k, ixx, iveg_ag, imm, k1, k2, ispc, imode
    INTEGER            :: kmt
    REAL               :: burnt_area, STD_burnt_area, dz_flam, dzi
    REAL               :: ztopmax(2)
    ...
    REAL    :: area           ! (DMK) scratch
    INTEGER :: maxtime        ! (DMK) scratch
    REAL    :: alpha          ! (DMK) scratch
    REAL    :: rsurf          ! (DMK) scratch
    REAL    :: fmoist         ! (DMK) scratch
    REAL    :: tdur           ! (DMK) scratch
    REAL    :: zsurf          ! (DMK) scratch
    REAL    :: heating(ntime) ! (DMK) scratch
    REAL    :: thtcon(nkp)    ! (DMK) scratch
    REAL    :: rvcon (nkp)    ! (DMK) scratch
    REAL    :: picon (nkp)    ! (DMK) scratch
    REAL    :: zcon  (nkp)    ! (DMK) scratch
    REAL    :: zzcon (nkp)    ! (DMK) scratch
    REAL    :: qv    (nkp)    ! (DMK) scratch
    REAL    :: qh    (nkp)    ! (DMK) scratch
    REAL    :: qi    (nkp)    ! (DMK) scratch
    REAL    :: qc    (nkp)    ! (DMK) scratch
    REAL    :: txs   (nkp)    ! (DMK) scratch
    REAL    :: cvi   (nkp)    ! (DMK) scratch
    REAL    :: vth   (nkp)    ! (DMK) scratch
    REAL    :: w     (nkp)    ! (DMK) scratch
    REAL    :: t     (nkp)    ! (DMK) scratch
    REAL    :: qsat  (nkp)    ! (DMK) scratch
    REAL    :: rho   (nkp)    ! (DMK) scratch
    REAL    :: radius(nkp)    ! (DMK) scratch
    REAL    :: visc  (nkp)    ! (DMK) scratch
    REAL    :: wc    (nkp)    ! (DMK) scratch
    REAL    :: wt    (nkp)    ! (DMK) scratch
    REAL    :: est   (nkp)    ! (DMK) scratch
    REAL    :: pe    (nkp)    ! (DMK) scratch
    REAL    :: te    (nkp)    ! (DMK) scratch
    REAL    :: tt    (nkp)    ! (DMK) scratch
    REAL    :: qvenv (nkp)    ! (DMK) scratch
    REAL    :: cvh   (nkp)    ! (DMK) scratch
    REAL    :: vti   (nkp)    ! (DMK) scratch
    REAL    :: pke   (nkp)    ! (DMK) scratch
    REAL    :: the   (nkp)    ! (DMK) scratch
    REAL    :: thve  (nkp)    ! (DMK) scratch
    REAL    :: dne   (nkp)    ! (DMK) scratch
    REAL    :: sc    (nkp)    ! (DMK) scratch 
    REAL    :: sct   (nkp)    ! (DMK) scratch 
!srf-AWE
    REAL    :: ucon  (nkp)    
    REAL    :: vcon  (nkp)    
    REAL    :: upe   (nkp)
...
!srf-AWE
    ...
    ! k 
    ! 1   => area media (m^2) dos focos  em biomas floresta dentro do gribox 
    !        i,j 
    ! 2   => area media (m^2) dos focos  em biomas savana dentro do gribox i,j
    ! 3   => area media (m^2) dos focos  em biomas pastagem dentro do gribox 
    !        i,j
    ...
    ! 7 a 9 =>  sem uso
    !10(=k_CO_smold) => parte da emissao total de CO correspondente a fase 
    !                   smoldering
...
!             zcon  (k)=zt_rams(k) *rtgt(i,j)    ! termod-point height
!             zzcon (k)=zm_rams(k) *rtgt(i,j)    ! W-point height
             zcon  (k)=zt_rams(k) *rtgt(i,j)    ! termod-point height
             zzcon (k)=zm_rams(k) *rtgt(i,j)    ! W-point height

	     zsurf = 0.
	     
             !if(i==43 .and. j==42)print*,'PL:',k,zcon(k),prcon (k),tmpcon(k)-273.,1000.*rvcon (k)
          ENDDO
          ...
          CALL get_env_condition(lpw(i,j),m1-1,kmt,qvenv,pke,the,te,pe,thve,dne,upe,vpe,vel_e, &
	                         zcon,zt,thtcon,rvcon,picon,ucon,vcon,zm,dzm,dzt,dz,zsurf,     &! n_setgrid,
                                 wind_eff,g,cp,cpor,p00,rgas)

          !- only for testing (using an external sounding)
          !xCALL get_env_condition_sound(lpw(i,j),m1-1,kmt,qvenv,pke,the,te,pe,thve,dne,upe,vpe,vel_e &
	  !x                      ,zcon,zt,thtcon,rvcon,picon,ucon,vcon,zm,dzm,dzt,dz,zsurf,n_setgrid,wind_eff)


          !- loop over the four types of aggregate biomes with fires
          DO iveg_ag=1,nveg_agreg !x

             !- verifica a existencia de emissao flaming para um bioma especifico
             !orig: if( plume( k_CO_smold + iveg_ag ,i,j) < 1.e-6 ) cycle
              IF(plume_mean_g(iveg_ag)%flam_frac(i,j) < 1.e-6 ) CYCLE !x

             ! burnt area and standard deviation
             !burnt_area   = 50.*1e4 ! only for testing with a sounding
             ...
             !- loop over the minimum and maximum heat fluxes allowed for each biome
             DO imm=1,2
                !--------------------
                !ixx=iveg_ag*10 + imm
                ixx=0+imm
                ...
                CALL makeplume(kmt,ztopmax(imm),ixx,imm,use_last,w,       &
	     ...
	     !stop 3333 !only for testing

             !- defines the vertical layer where the flaming emission will
             !  be released in the 3d atmosph model
             CALL set_flam_vert(ztopmax,k1,k2,zzcon,W_VMD,VMD)

             !- thicknees of the vertical layer
...
!                        (rtgt(i,j)/dzt_rams(2)                 )*   &
                   ...
                   chem1_src_g(it1,bburn,ispc)%sc_src(k,i,j) = &
                        chem1_src_g(it1,bburn,ispc)%sc_src(k,i,j) +&
...
!                     q_smold_kgm2 = (rtgt(i,j)/dzt_rams(2)                 )*   &
                      ...
                      aer1_g(imode,aer_bburn)%sc_src(k,i,j) = &
                           aer1_g(imode,aer_bburn)%sc_src(k,i,j) +&
                           plume_mean_g(iveg_ag)%flam_frac(i,j)  *&
                           q_smold_kgm2 * &
                           dzi !use this in case the emission src is tracer density
...
!!$    REAL,    INTENT(INOUT) :: zcon  (:)
    ...
    IF( n_setgrid == 0) THEN
       ...
       CALL set_grid(zt,zm,dzm,dzt,dz,zsurf) 
...
!!$    CALL htint(nk, rvcon,zcon(k1:nk+k1-1),kmt,qvenv,zt)
  ...
  SUBROUTINE set_grid(zt,zm,dzm,dzt,dz,zsurf)
...
!!$    REAL, INTENT(INOUT) :: dzt(nkp)
  ...
  SUBROUTINE set_flam_vert(ztopmax,k1,k2,zzcon,W_VMD,VMD)
...
!!$    REAL,    INTENT(OUT) :: VMD(nkp,2)
  ...
  SUBROUTINE get_fire_properties(imm,iveg_ag,burnt_area,use_last, &
    ...
    ! plumegen_coms
!!$    REAL,    INTENT(INOUT) :: heating(ntime) ! (DMK) alt. (OUT) para (INOUT)
    REAL,    INTENT(INOUT) :: heating(:) ! (DMK) alt. (OUT) para (INOUT)
    REAL,    INTENT(INOUT) :: area           ! (DMK) alterado (OUT) para (INOUT)
    INTEGER, INTENT(INOUT) :: maxtime        ! (DMK) alterado (OUT) para (INOUT)
...
!!$         CALL FatalError(h//" Increase time duration (ntime) in min - see file chem_plumerise_scalar.f90")
  ...
  !----------------------------------------------------------------------------
  !
  SUBROUTINE MAKEPLUME(kmt,ztopmax,ixx,imm,use_last,w,t,pe,qsat, &
...
!srf-awe    
    ...
    IF(imm==1 .AND. use_last==1)  &
         CALL INITIAL(kmt,txs,w,t,wc,wt,qv,vth,vti,qh,qi,qc,est,qsat,rho, &
                      ...
                      mintime,fmoist,time,VEL_P,rad_p)
    IF(use_last/=1) &
         CALL INITIAL(kmt,txs,w,t,wc,wt,qv,vth,vti,qh,qi,qc,est,qsat,rho, &
                      radius,visc,te,qvenv,rsurf,alpha,zt,viscosity,pe,   &
                      area,dt,l,wbar,dqsdz,cvi,tdur,heating,    &
