...
!    REAL, ALLOCATABLE, DIMENSION(:,:) :: src_dummy_2d
    ...
    DO i=1,nspecies*nsrc*5
       ...
       IF (mchnum==master_num) READ(iunit,*,IOSTAT=exDo) section
       IF (mchnum==master_num) THEN
	     ...
	     iax=1
          END IF
          IF(TRIM(section) == 'plume' .AND. plumerise /= on .and. ipx==0) THEN
...
!         CALL vfirec(iunit,chem1_src_g(itim,isrc,ispc)%sc_src(1,:,:),m2*m3,'LIN')
	  ...
	  print*,"   chem max/min:",maxval(chem1_src_g(itim,isrc,ispc)%sc_src(1,1:m2,1:m3)),&
...
!
       ...
       ELSEIF(TRIM(section) == 'aerosol' ) THEN 
          ...
          IF (mchnum==master_num) THEN
             ...
             IF(trim(read_spc_name)=='V_ASH1' .and. ispc==v_ash) then
              PRINT*,'-> reading size distr for ', TRIM(spc_aer_name(imode,ispc)), ' source: ', TRIM(src_name(isrc))
	      READ(iunit,*)   cdummy
	  ...
	  !- broadcast of the mass distribution of volcanic ash
          ...
          !PRINT*,'proc ',TRIM(ADJUSTL(c2)),' ->reading emission of ', &
	  !       TRIM(spc_aer_name(imode,ispc)), ' source: ', TRIM(src_name(isrc))
          
	  CALL ReadStoreFullFieldAndOwnChunk(ng,iunit,oneGlobalEmissData(ng)%src_dummy_2d, &
                                             src_dummy_2d,                               &
                                             TRIM(spc_aer_name(imode,ispc))//"_"//TRIM(src_name(isrc)))
          !
	  ...
	  IF( AEROSOL > 0) THEN
		 ...
		 print*,"   aer  max/min:",maxval(aer1_g(imode,ispc)%sc_src(1,1:m2,1:m3)),&
          ...
          ! aer1_g(imode,ispc)%sc_src(1,1:m2,1:m3) = src_dummy_2d(1:m2,1:m3)
	  !print*,size(aer1_g(imode,ispc)%sc_src,1),size(aer1_g(imode,ispc)%sc_src,2),&
	  !       size(aer1_g(imode,ispc)%sc_src,3),m1,m2,m3
	  !print*, 'maxval aer=',imode, ispc,maxval(aer1_g(imode,ispc)%sc_src(1,1:m2,1:m3)),maxval(src_dummy_2d)
...
! -alterando a distribuicao de concentraçao de cinzas 29/12/2013
