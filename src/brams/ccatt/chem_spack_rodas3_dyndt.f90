...
!!  end if
  ...
  DO i=1,nob !- loop over all blocks
    ...
    DO ijk=1,block_end(i) !index_g%block_end(i)
      ...
      !- volmol é usado para converter de ppbm para molecule/cm^3 (fator fxc incluido)
    ...
    IF(split_method == 'PARALLEL' .and. N_DYN_CHEM > 1) then 
	 ...
	 kij_ =kij_index(ijk,i) !index_g%kij_index(ijk,i)!- kij brams tendency index
	   ...
	   !- get back the concentration at the begin of the chemistry timestep integration:
           ...
           !spack(inob)%sc_p_4(ijk,n) =(chem1_g(n)%sc_p(k_,i_,j_) - beta*chem1_g(n)%sc_t_dyn(kij_)*N_DYN_CHEM*dtlt)& !dtlt é o dinamico
           !				* spack(inob)%volmol(ijk)/weight(n)
         END DO 	 
      END DO	   
    ELSE
...
!    compute DLK2 by solving (1/Igama*dt - DLRDC)DLK2 = 4/h * DLK1 +  DLb1 
     ...
     DO ijk=1,block_end(i) !index_g%block_end(i)
        CALL Solve_linear(nspecies,spack_2d(ijk,inob)%DLmat& !matrix  I /( gama dt)- Jac
	                  	  ,spack_2d(ijk,inob)%DLb2 & !matrix independent (P-L)
			   	  ,spack_2d(ijk,inob)%DLk2 ) !matrix solution
			   	  !,i)  	        
     ENDDO
...
!    c) compute   K3 by solving (1 /(0.5 h) - JAC)K3 =   F(Y3) + 0.5 (K1-K2)
     ...
     DO ijk=1,block_end(i) !index_g%block_end(i)
        CALL Solve_linear(nspecies,spack_2d(ijk,inob)%DLmat& !matrix  I/(gama dt)- Jac
	                  	  ,spack_2d(ijk,inob)%DLb3 & !matrix independent 
			   	  ,spack_2d(ijk,inob)%DLk3 ) !matrix solution
			   	  !,i)  	        
     ENDDO
...
!    c) compute   K4 by solving (1/(0.5 h)- JAC)K4 =    F(Y3) + K1/h -K2/h -8/3 K3/h
     ...
     DO ijk=1,block_end(i) !index_g%block_end(i)
        CALL Solve_linear(nspecies,spack_2d(ijk,inob)%DLmat& !matrix  I/(gama dt)- Jac
	                  	  ,spack_2d(ijk,inob)%DLb4 & !matrix independent 
			   	  ,spack_2d(ijk,inob)%DLk4 ) !matrix solution
			   	  !,i)  	        
     ENDDO
