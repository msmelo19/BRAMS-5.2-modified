...
!srf REAL	   ::  PAPH(mgmxp,mgmzp+1)  ! PRESSAO (hPa)
 ...
 REAL	           ::  PRMISS(mgmxp,mgmzp)  ! RAZÃO DE MISTURA DE SATURAÇÃO 
 REAL	           ::  PRMISSL(mgmxp,mgmzp) ! RAZÃO DE MISTURA DE SATURAÇÃO 
 ...
 REAL              ::  ZWU2H(mgmxp,mgmzp)
   ...
   DO JL=KIDIA,KFDIA
       DO JK=KLEV, 1, -1  ! INVERTIDO PARA FICAR COMPATÍVEL COM O ESQUEMA DE GRELL !
	 JKE = KLEV+1-JK
...
!----------------------------------------------------------------------
  ...
  INTEGER :: IDPL(mgmxp)  
  INTEGER :: ierr(mgmxp)   ! flag que passa ldcum (lógica) para o inteiro correspondente 
...
!                  -------------------------------
!----------------------------------------------------------------------
!      CONTEÚDO MÍNIMO DE ÁGUA PARA ENCONTRAR A BASE DA NUVEM! CLÁUDIO SILVA
		 ...
		 DO JL=KIDIA,KFDIA
 		 ...
 		 IF(IS /= 0) THEN
			...
			IF(JKK == KLEV) THEN
			 ...
			 ! Esabilizando ainda mais as parcelas, fazendo a Camada de mistura até 80 hPa
...
!srf			      ZMIX(JL)= ENTRPEN           *(PGEOH(JL,JK)-PGEOH(JL,JK+1))/RG
			...
			! O CÁLCULO DA BASE DA NUVEM DEPENDE EXCLUSIAMENTE DE ZLU !
...
!*********************************************
            IF (LPHYLIN) THEN ! para usar a física não linearizada (Cláudio Silva)
!*********************************************                 
