...
!!$     vpref=varpref
       ...
       do nf=1,nvarffiles
          ...
          !--(DMK-CCATT-OLD)-----------------------------------------------------
          !        call makefnam (sVarName, varpref, 0, iyears, imonths, idates,  &
          !             ihours, 'V', '$', 'tag')      
          !--(DMK-CCATT-FIM)-----------------------------------------------------
          
	  !print*,"looking for:",sVarName(1:len_trim(sVarName))
          !
...
!!$     PRINT*,'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
	...
	!  endif
      ...
      !    !-- SRF added to reuse ivar files with chemical fields but
      !	  !-- only reading the meteo fields (for meteo runs only).
      ! if(chem_assim == 0 .and. chemistry == -1 ) then 
      !    do nspc=1,nspecies
      !       if (nspc==1 .and. mchnum == master_num) then
      !         print*,'--------------------------------------------------------------------------'
      !         print*,'Skipping chemical fields since this run is only with meteorology '
      !       endif
      !       if(spc_alloc(fdda,nspc) == 1) call vfirec(iun,scratch2,nxyzp,'LIN')
      !    enddo
      !    if (mchnum == master_num) &
      !       print*,'--------------------------------------------------------------------------'
      ! endif
