...
module meteogram
   ...
   do np = 1, size(polygons)
      ...
      if(globalPts .gt. 0)then
         ...
         do while(associated(auxPixel))
            ...
            do nv = 1, nVarTables
               !DSM auxMeteoPoly%vertices(v)%vTableName = varNamesInput(nv)
 ...
 subroutine ProcessLocalMeteogram(meteoPolys)
   ...
   character(len=450), dimension(:), allocatable :: cityName
   ...
   character(len=10), parameter, dimension(nvarOutput) :: varNamesOutput = (/'U10MJ     ', &
                                                                             'V10MJ     ', &
                                                                             'T2MJ      ', &
                                                                             'TD2MJ     ', &
                                                                             'SLPMETAR  ', &
   ...
   do while(associated(auxMeteoPoly))
	...
	cityName(city)  = auxMeteoPoly%name
   ...
   if(mchnum .eq. master_num)then
      ...
      write(meteogramOutUnit,*) '                                polygon_name - st points'//&
                                '    Lat        Lon       Uave    Umax    Umin    '//  &
                               'Vave    Vmax    Vmin Tave   Tmax  Tmin TDave TDmax '//&
                               'TDmin SLPave SLPmax SLPmin PRECave PRECmax PRECmin'
   	do city = 1, nCities
         write(meteogramOutUnit, &
            '(a60,1x,i6,1x,f9.5,1x,f10.5,1x,6(f7.2,1x),6(f5.1,1x),3(f6.1,1x),3(f6.2,1x))') &
             trim(cityName(city))  &
