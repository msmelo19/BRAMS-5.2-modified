...
! Complete Exner tendency and vertical coordinate.
  IF (IEXEV .EQ. 2 .AND. IF_ADAP .NE. 0) THEN
     PRINT *, 'FATAL - IEXEV cannot be set to 2 with shaved-eta coordinate'
     IFATERR=IFATERR+1 
  ENDIF
!srf
  IF (IEXEV .EQ. 2 .AND. level .EQ. 0) THEN
     PRINT *, 'FATAL - IEXEV cannot be set to 2 with microphyics level = 0 '
     IFATERR=IFATERR+1 
  ENDIF
...
!    IFATERR=IFATERR+1
!   endif
   if( (mcphys_type == 2 .or. mcphys_type == 3) .and. nnqparm(NGR) ==2) then
    print*,' FATAL - cannot use nnqparm = 2 scheme with GT microphysics'
    IFATERR=IFATERR+1
   endif
enddo
...
!  endif
 ...
 ENDIF
  ...
  ![MLO - Blocking Grell deep/shallow convection without TKE
     if (nnshcu(ng) == 2 .or. NNQPARM(NG) == 2  .or. NNQPARM(NG) == 5) then
        if (idiffk(ng) == 2 .or. idiffk(ng) == 3) then
           print *, 'FATAL - deep (nnqparm 2 or 5) and shallow (nnshcu 2) retuires turbulence scheme with TKE (1,4,5,6,7)'
