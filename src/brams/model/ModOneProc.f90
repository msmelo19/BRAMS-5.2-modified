...
!!$  end if
    ...
    if (runtype(1:7)=='MAKESFC' .or. runtype(1:9)=='MAKEVFILE') then
       if (oneParallelEnvironment%nmachs > 1) then
          call fatal_error(h//" BRAMS MAKESFC and MAKEVFILE runs "//&
               "use a single MPI process; re-submit with number "//&
               "of processes = 1")
       end if
    end if
    ...
    call MakeSfcfiles()
    !print *,'LFR-DBG inside oneproc 1: ',nodemxp(mynum,1); call flush(6)
    ...
    if (runtype(1:7)=='MAKESFC') then
       ...
       if (mchnum==master_num) then
	        ...
	        write(OUTPUT_UNIT,"(' ISAN complete ')")
	     endif
       endif
...
!    print *,'LFR-DBG inside oneproc 2: ',nodemxp(mynum,1); call flush(6)
...
!    print *,'LFR-DBG inside oneproc 2,5: ',nodemxp(mynum,1); call flush(6)        
...
!    print *,'LFR-DBG inside oneproc 3: ',nodemxp(mynum,1); call flush(6)       
       if(ioutput == 5)then
	...
	ioutput = 2
       ...
       call InitFields(1)
       !print *,'LFR-DBG inside oneproc 5: ',nodemxp(mynum,1); call flush(6)
       ...
       do while (time<timmax)
          ...
          if (isendbackflg==1) then
             ...
             if (isendsrc==1) then
                do ifm = 1, ngrids
                   call newgrid(ifm)
                   print*,'-> chem/aer sources maps reading: ',time/3600.,' grid=',ifm
          ...
          if (RUNTYPE/='POS') then
             ...
             do npass=1,nsubs
                ...
                if ( ( dyncore_flag == 0 ) .or. ( dyncore_flag == 1 ) ) then
                  ...
                  call fatal_error("ERROR in subroutine OneProc: value of dyncore_flag is unknown")
                end if
