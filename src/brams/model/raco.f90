...
!call flush(6)
  ...
  subroutine acoustic_new(OneGrid, nnacoust_loc)
...
!! Please, read @link https://creativecommons.org/licenses/GPL/2.0/legalcode.pt
    ...
    !MB:
    !nmbr_gpts = mxp * myp * mzp    !MB: only for testing!!!
    !write(*,"(A,I2,4F15.8)") "u    : ", 12, minval( up ), maxval( up ), sum( up )/nmbr_gpts, up(7,8,9)
    !write(*,"(A,I2,4F15.8)") "v    : ", 12, minval( vp ), maxval( vp ), sum( vp )/nmbr_gpts, vp(7,8,9)
    !write(*,"(A,I2,4F15.8)") "w    : ", 12, minval( wp ), maxval( wp ), sum( wp )/nmbr_gpts, wp(7,8,9)
    !write(*,"(A,I2,4F15.8)") "pi   : ", 12, minval( pp ), maxval( pp ), sum( pp )/nmbr_gpts, pp(7,8,9)
    ...
    !MB:
    !write(*,"(A,I2,4F15.8)") "u    : ", 23, minval( up ), maxval( up ), sum( up )/nmbr_gpts, up(7,8,9)
    !write(*,"(A,I2,4F15.8)") "v    : ", 23, minval( vp ), maxval( vp ), sum( vp )/nmbr_gpts, vp(7,8,9)
    !write(*,"(A,I2,4F15.8)") "w    : ", 23, minval( wp ), maxval( wp ), sum( wp )/nmbr_gpts, wp(7,8,9)
    !write(*,"(A,I2,4F15.8)") "pi   : ", 23, minval( pp ), maxval( pp ), sum( pp )/nmbr_gpts, pp(7,8,9)

    !write (*,fmt='("Pos-loop:",I2.2, '//ctime//'(E9.3,1X))') mynum,(pp(k,2,2),k=1,mzp); call flush(6)
  ...
  subroutine init_div_damping_coeff( dts )
    ...
    !MB:
    print*, "div_damp_strength=", div_damp_strength, &
            "  div_damp_slope=", div_damp_slope,  &
            limit_alpha_div_by_slope_stability
...
!! Please, read @link https://creativecommons.org/licenses/GPL/2.0/legalcode.pt
  ...
  call boyanc   (mzp,mxp,myp,ia,iz,ja,jz,level,mynum, wt )
...
subroutine boyanc(mzp,mxp,myp,ia,iz,ja,jz,level,mynum, wt )
!> @brief: boyancy
...
!! Please, read @link https://creativecommons.org/licenses/GPL/2.0/legalcode.pt
  ...
  integer,intent(in) :: mzp,mxp,myp,ia,iz,ja,jz,level,mynum

  real, dimension(mzp,mxp,myp), intent(inout) :: wt
  real, dimension(mzp,mxp,myp) :: vtemp
  ...
  do j=1,myp
    do i=1,mxp
      lpw(i,j)=int(grid_g(ngrid)%lpw(i,j))
      do k=1,mzp
        n=n+1
        !wt(k,i,j)=tend%wt(n) 
	!wt(k,i,j)=wt_ptr(n)
        vtemp(k,i,j)=scratch%vt3da(n)  !MB: necessary???
      end do
    enddo
  enddo
  
  !- lpw is an integer dummy array
  !- grid_g(ngrid)%lpw(:,:) is a real (not integer) array
  lpw(:,:)=int(grid_g(ngrid)%lpw(:,:))

  if (level .ge. 1) then
     do j = ja,jz
        do i = ia,iz
           do k = lpw(i,j),mzp-1
              vtemp(k,i,j) = gg * ((basic_g(ngrid)%theta(k,i,j) * (1. + .61 * basic_g(ngrid)%rv(k,i,j))  &
                           - basic_g(ngrid)%th0(k,i,j)) / basic_g(ngrid)%th0(k,i,j) - &
			   (basic_g(ngrid)%rtp(k,i,j) - basic_g(ngrid)%rv(k,i,j)) )
           enddo
        enddo
     enddo
  else
     do j = ja,jz
        do i = ia,iz
           do k = lpw(i,j),mzp-1
              vtemp(k,i,j) = gg * (basic_g(ngrid)%theta(k,i,j) / basic_g(ngrid)%th0(k,i,j) - 1.)
           enddo
        enddo
     enddo
  endif
  ...
  do j=1,myp
    do i=1,mxp
      do k=1,mzp
        ...
        !tend%wt(n)=wt(k,i,j)
	!wt_ptr(n) = wt(k,i,j)
        scratch%vt3da(n)=vtemp(k,i,j)    !MB: necessary???
      end do
    enddo
  enddo
