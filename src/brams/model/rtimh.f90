...
!!$  call TimeStamp(TS_PHYSICS)
  ...
  IF( NNQPARM(ngrid) >=2 .OR. NNSHCU(ngrid) >=2 )CALL prepare_lsf(NNQPARM(ngrid), NNSHCU(ngrid),2)
...
!!$  call TimeStamp(TS_PHYSICS)
  ...
  !print *, 'LFR-DBG Rtimh 05: ',associated(g3d_g(ngrid)%thsrc); call flush(6)
  if (iexev == 2) &
  ...
  IF( NNQPARM(ngrid) >=2 .OR. NNSHCU(ngrid)>=2 ) CALL prepare_lsf(NNQPARM(ngrid), NNSHCU(ngrid),3)
...
!----- for R-Ks time integration ---
  ...
  IF(  NNQPARM(ngrid) >=2 .OR. NNSHCU(ngrid)>=2 ) CALL prepare_lsf(NNQPARM(ngrid), NNSHCU(ngrid),4)
!!$  call TimeStamp(TS_DINAMICS)
  ...
  ELSEIF(acoshdp == 1 ) THEN
     ...
     !                    Shallow Convection scheme
        ...
        CALL prepare_lsf(NNQPARM(ngrid), NNSHCU(ngrid),5)
...
!!$  call TimeStamp(TS_PHYSICS)
  ...
  call acoustic_new(OneGrid, nnacoust(ngrid) )   !MB:
