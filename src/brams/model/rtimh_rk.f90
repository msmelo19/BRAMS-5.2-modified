...
subroutine timestep_rk(OneGrid)
  ...
  elseif (isfcl == 5) then
     if (time==0.) &
     call SFCLYR      (mzp,mxp,myp,ia,iz,ja,jz,ibcon)
     call SFCLYR_JULES(mzp,mxp,myp,ia,iz,ja,jz,jdim)
  !DSM}
#endif
  endif
...
!!$  call TimeStamp(TS_PHYSICS)
  ...
  IF( NNQPARM(ngrid) >=2 .OR. NNSHCU(ngrid)>=2 ) CALL RK_prepare_lsf(NNQPARM(ngrid), NNSHCU(ngrid),1)
...
!- no futuro, tente  rk_nmbr_small_timesteps(1)=1 e dts=2* dtlt / nnacoust(ngrid)
  ...
  do l_rk = 1, rk_order
     ...
     if ( .not. flag_mb_adv_test) then
       call acoustic_new(OneGrid, rk_nmbr_small_timesteps(l_rk) )
     else
      ! just for testing:
     ...
     !write(*,"(A,I2,3F15.8)") "ut_rk  ", l_rk, minval( tend%ut_rk  ), maxval( tend%ut_rk  ), sum( tend%ut_rk  )/nmbr_gpts
     !write(*,"(A,I2,3F15.8)") "vt_rk  ", l_rk, minval( tend%vt_rk  ), maxval( tend%vt_rk  ), sum( tend%vt_rk  )/nmbr_gpts
     !write(*,"(A,I2,3F15.8)") "wt_rk  ", l_rk, minval( tend%wt_rk  ), maxval( tend%wt_rk  ), sum( tend%wt_rk  )/nmbr_gpts
     !write(*,"(A,I2,3F15.8)") "pt_rk  ", l_rk, minval( tend%pt_rk  ), maxval( tend%pt_rk  ), sum( tend%pt_rk  )/nmbr_gpts
     !write(*,"(A,I2,3F15.8)") "tht_rk ", l_rk, minval( tend%tht_rk ), maxval( tend%tht_rk ), sum( tend%tht_rk )/nmbr_gpts
     !
     !MB:
     !write(*,"(A,I2,4F15.8)") "uc  ", l_rk, minval( basic_g(ngrid)%uc  ), maxval( basic_g(ngrid)%uc  ), &
     !    sum( basic_g(ngrid)%uc  )/nmbr_gpts, basic_g(ngrid)%uc(7,8,9)
     !write(*,"(A,I2,4F15.8)") "vc  ", l_rk, minval( basic_g(ngrid)%vc  ), maxval( basic_g(ngrid)%vc  ), &
     !    sum( basic_g(ngrid)%vc  )/nmbr_gpts, basic_g(ngrid)%vc(7,8,9)
     !write(*,"(A,I2,4F15.8)") "wc  ", l_rk, minval( basic_g(ngrid)%wc  ), maxval( basic_g(ngrid)%wc  ), &
     !     sum( basic_g(ngrid)%wc  )/nmbr_gpts, basic_g(ngrid)%wc(7,8,9)
     !write(*,"(A,I2,4F15.8)") "pc  ", l_rk, minval( basic_g(ngrid)%pc  ), maxval( basic_g(ngrid)%pc  ), &
     !    sum( basic_g(ngrid)%pc  )/nmbr_gpts, basic_g(ngrid)%pc(7,8,9)
     !write(*,"(A,I2,4F15.8)") "thc ", l_rk, minval( basic_g(ngrid)%thc ), maxval( basic_g(ngrid)%thc ), &
     !    sum( basic_g(ngrid)%thc )/nmbr_gpts, basic_g(ngrid)%thc(7,8,9)
