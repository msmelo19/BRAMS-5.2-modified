...
subroutine gather_cpu_time_master_print(master_num, mynum, nmachs, tcpu)
 ...
 subroutine GatherMeteogram(masterNum, myNum, nPts, nVars, localSums, localMins, localMaxs, fUnit, totalSums, totalMins, totalMaxs)
   implicit none
   integer, intent(in)          :: masterNum
   integer, intent(in)          :: myNum
   integer, intent(in)          :: nPts
   integer, intent(in)                            :: nVars
   real, dimension(nVars), intent(in)             :: localSums
   real, dimension(nVars), intent(in)             :: localMaxs
   real, dimension(nVars), intent(in)             :: localMins
   integer, intent(in)                            :: fUnit
   real, dimension(nVars), intent(out)            :: totalSums
   real, dimension(nVars), intent(out)            :: totalMins
   real, dimension(nVars), intent(out)            :: totalMaxs
    ...
    !call MPI_Barrier(MPI_COMM_WORLD,ierr)
      ...
      do nv = 1, nVars
       ...
       call MPI_Reduce(localSums(nv), totalSums(nv), 1, MPI_REAL, MPI_SUM, masterNum, MPI_COMM_WORLD, ierr)
       call MPI_Reduce(localMaxs(nv), totalMaxs(nv), 1, MPI_REAL, MPI_MAX, masterNum, MPI_COMM_WORLD, ierr)
       call MPI_Reduce(localMins(nv), totalMins(nv), 1, MPI_REAL, MPI_MIN, masterNum, MPI_COMM_WORLD, ierr)   
