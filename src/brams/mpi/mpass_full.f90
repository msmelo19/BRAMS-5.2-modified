  ...
  do ng = 1, ngrids
     do idim_type=2,7
        call SizesDisplacements (idim_type, nmachs, &
             ...
             nodemxp(1,ng), nodemyp(1,ng), nnzp(ng), &
             nodeia(1,ng), nodeiz(1,ng), nodeja(1,ng), nodejz(1,ng), &
             nodeibcon(1,ng),il1(1,idim_type), ir2(1,idim_type), &
             jb1(1,idim_type), jt2(1,idim_type),eachSizeGathered(1,idim_type),&
             displacement(1,idim_type))
     end do
  end do
...
!!$     if (writeAnlFile) then
     ...
     call AnlwrtInitialize(varType,anamel,anamelh(1:len_trim(anamelh)),timeold)
  ...
  do ng = 1, ngrids
     ...
     if (mchnum == master_num) then
        call OpenAnlwrt(anamel(ng)(1:len_trim(anamel(ng))))
        npointer = 0_i8 
        nvcnt = 0
     end if
        ...
        if (WillGather(nv,ng)) then
           ...
           elseif(varn == 'HKM') then
              ...
              call RAMS_aprep_hkh (nodemxyzp,field &
                   ,turb_g(ng)%vkh(1,1,1),basic_g(ng)%dn0(1,1,1)  &
                   ,scr1(1),idiffk(ng),xkhkm(ng))
              varn='HKH'
              field =>scr1(1)
           ...
           call OneGatherStoreAllChunks(mchnum, mynum, nmachs, &
                ...
                eachSizeGathered(1,idim_type), displacement(1,idim_type), &
                nnxp(ng), nnyp(ng), nnzp(ng), nzs, nzg, npatch, nwave, &
                nodemxp(1,ng), nodemyp(1,ng), nnzp(ng), &
                nodeia(1,ng), nodeiz(1,ng), nodeja(1,ng), &
                nodejz(1,ng), nodei0(1,ng), nodej0(1,ng), &
                il1(1,idim_type), ir2(1,idim_type), &
                jb1(1,idim_type), jt2(1,idim_type), &
                sizeRes, Res(1))
  ...
  if (mchnum == master_num) then
     call AnlwrtFinalize(varType,anamelh(1:len_trim(anamelh)),anamel(1)(1:len_trim(anamel(1))),ioaunt,iclobber,nvcnt, &
