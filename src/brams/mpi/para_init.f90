...
subroutine domain_decomposition_dump(fUnit)
  ...
  write(fUnit,"(a30,i8.8,a12,i8.8,a11)") ' === Domain decomposition for ',&
       ngrids,' grids with ',nmachs,' cores ==='

  write(fUnit,"(a)") '      grid    node   x-beg   x-end   y-beg   y-end      cols'
  do ngr = 1, ngrids
     do jnode = 1,nmachs
        ncols= (1+ixe(jnode,ngr)-ixb(jnode,ngr))  &
             *(1+iye(jnode,ngr)-iyb(jnode,ngr))
        write(fUnit,"('  ',7i8)") ngr,jnode,ixb(jnode,ngr),ixe(jnode,ngr)  &
