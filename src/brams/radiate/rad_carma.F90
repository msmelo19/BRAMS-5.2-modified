MODULE rad_carma
  ...
  USE mem_carma, ONLY: &
       ...
       RAIN_aerad, tabove_aerad, gc, cdec, pc, press, p_top, &
       dpg, tt, rdh2o, pah2o, paco2, pao2, pao3, h2ocol_aerad, &
       taugas, paray, qv_aerad, pc_aerad, caer, rsfx, emisir_aerad, &
       lla, lls, fnet, directu, heats_aerad, heati_aerad, solnet, &
       xirdown, tauaer, qv_aerad, direc, taul, w0, g0, opd, uopd, &
       y3, ptempg, ptempt, ptemp, slope, u1i, b1, b2, ak, GAMI, &
       ee1, el1, em1, el2, em2, af, bf, ef, b3, cpb, cp, cmb, &
       ck1, ck2, tmi, aotmap_t, &

!--(DMK-CCATT-INI)-------------------------------------------------------------------
       ntotal_aer, tauaer_x, wol_x, gol_x, &
!--(DMK-CCATT-FIM)-------------------------------------------------------------------
...
! (DMK 07-05-2013) Correcao no CCATT-4.3.3, variavel nao inicializada
!                  Se desativado, gera perda de reprodutibilidade na temperatura e 
!                  precipitacao no Operacional 5km
    aotl = 0.0
    pmr  = 0.0
...
!---srf incluir instrucoes para o MATRIX
	...
	if(cosi .gt. 0)then
	     !rshort(ij) = rshort(ij)* cosi / cosz(ij) !Previous unrealistic radiation field along Andes
	     rshort(ij) = rshort(ij)                   !NER to be fully test.
	else
	     !rshort(ij) = 0            !Previous
	     rshort(ij) = rshort(ij)   !NER
	end if        	     
!--(DMK-CCATT-OLD)-----------------------------------------------------
...
!    if(n_aer == 1) then ! bburn 
!  	 r0(:,:)   = 2.10e-5
  	 rsig(:,:) = 1.55
!	 AER_DENS(n_aer)= 1.39
!    elseif(n_aer == 2) then ! urban
! 	 r0(:,:)   = 2.00e-5
!  	 rsig(:,:) = 1.50
!	 AER_DENS(n_aer)=1.8
!    elseif(n_aer == 3) then ! continental background
!  	 r0(:,:)   = 1.80e-5
!  	 rsig(:,:) = 2.
!	 AER_DENS(n_aer)=2.2
!    else
!     stop 'n_aer rad carma >3'
!    endif
!    rhop3(1,1,1)=AER_DENS(n_aer)
...
!--(DMK-CCATT-FIM)-------------------------------------------------------------------
		...
		if(aotMap(ij) .ne. 0)then
			currSite = aotMap(ij)
		else
		
			if(xland_aerad(ij) .gt. .009)then
				currSite = 11
			else
				currSite = 12
			end if
  	 ...
  	 r0(:,:)  = 1.e-4 * caseR(idaot(ij),currSite)
  	 pdens(:,:)  = caseD(idaot(ij),currSite)
...
!--(DMK-CCATT-INI)-------------------------------------------------------------------
      ...
      sol(l)  =  solfac * solfx(nprob(l)) * weight(l)
!--(DMK-CCATT-OLD)-------------------------------------------------------------------
...
!            aot11=0.1          !TMP KML2
		...
		!at moment, just one optical model per site!		
		idaot(ij) = 1
...
!-kml-27092011         				   caer(ij,j,i,ig)
			...
			!if (taua(ij,l,j) .gt. 0.0001) print*,'taua=',taua(ij,l,j),'xsecta=',xsecta(i,ig)			
		        
!			tauaer(ij,l,j) = MAX(taua(ij,nprob(l),j),REAL(epsilon))	
!-kml-27092011
!			tauaer(ij,l,j) = MAX(taua(ij,l,j),REAL(epsilon))	!ner_dez10
...
!--(DMK-CCATT-FIM)----------------------------------------------------------  
         		...
         		gol(ij,l,j)    = gonew(ij,nprob(l))
               		!if(tauaer(ij,l,j)>0.1) then
!			if(caer(ij,j,i,ig)> 0.1) then

			  !print*,'ij,j,nprob=',ij,l,nprob(l),xsecta(i,ig)
...
!--(DMK-CCATT-INI)----------------------------------------------------------  
	   ...
	   tauaer_x(1:iend,1:ntotal,1:nlayer,n_aer) =taua  (1:iend,1:ntotal,1:nlayer) 
	   wol_x   (1:iend,1:ntotal,1:nlayer,n_aer) =wol   (1:iend,1:ntotal,1:nlayer) 
           gol_x   (1:iend,1:ntotal,1:nlayer,n_aer) =gol   (1:iend,1:ntotal,1:nlayer) 
	   ! kml- saving properties by aerosol type
!--(DMK-CCATT-FIM)----------------------------------------------------------  
...
!!
!! Desativado: reprodutibilidade na temperatura e precipitacao da ver operacional, mas
!!             perda de reprodutibilidade na aot550 da ver 4.3.3
!!
...
!!          na temperatura e precipitacao da ver operacional
   ...
   ELSE
     ...
     do i=1,ntotal_aer
        tauaer(:,:,:)=tauaer(:,:,:)+ tauaer_x(:,:,:,i)      
     enddo
     do i=1,ntotal_aer
     wol(:,:,:)=wol(:,:,:)+wol_x(:,:,:,i)*tauaer_x(:,:,:,i) 
     gol(:,:,:)=gol(:,:,:)+gol_x(:,:,:,i)*tauaer_x(:,:,:,i) 
     enddo
     wol(:,:,:)=wol(:,:,:)/tauaer(:,:,:) 
     gol(:,:,:)=gol(:,:,:)/tauaer(:,:,:) 
     ! kml- saving properties by aerosol type
