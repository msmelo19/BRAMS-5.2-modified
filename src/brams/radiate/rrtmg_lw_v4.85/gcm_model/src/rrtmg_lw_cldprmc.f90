...
!             (cwp .ge. cldmin .or. taucmc(ig,lay) .ge. cldmin)) then
      ...
      do lay = 1, nlayers
          cwp = ciwpmc(lay) + clwpmc(lay)
        do ig = 1, ngptlw
         if (cldfmc(lay) .ge. cldmin .and. &
             (cwp .ge. cldmin .or. taucmc(ig,lay) .ge. cldmin)) then
!kml
...
!kml
               ...
               elseif (liqflag .eq. 1) then
                  ...
                  if (radliq .lt. 2.5_rb .or. radliq .gt. 60._rb) stop &
                       'LIQUID EFFECTIVE RADIUS OUT OF BOUNDS at routine cldprmc'
...
!                                clwpmc(ig,lay) * abscoliq(ig)
               taucmc(ig,lay) = ciwpmc(lay) * abscoice(ig) + &
                                clwpmc(lay) * abscoliq(ig)
