...
! For iceflag=3 option, ice particle generalized effective size is limited to 5.0 to 140.0 microns
		      ...
		      print*,"radice=",radice
		      stop 'ICE GENERALIZED EFFECTIVE SIZE OUT OF BOUNDS in LW'
		  endif
                     ncbands = 16
                     factor = (radice - 2._rb)/3._rb
                     index = int(factor)
...
! Calculation of absorption coefficients due to water clouds.
               ...
               elseif (liqflag .eq. 1) then
                  radliq = rel(lay)
!                  print *, 'LFR-Radice: ',lay,radliq,rel(lay); CALL flush(6)
                  if (radliq .lt. 2.5_rb .or. radliq .gt. 60._rb) then 
                      print*,'LIQUID EFFECTIVE RADIUS OUT OF BOUNDS at routine cldprop (lw)'
		      print*,"radliq=",radliq, "bounds 2.5 - 60."
		      stop 
		  endif
