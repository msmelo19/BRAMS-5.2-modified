...
!     real(kind=rb) :: clwpmc(ngptlw,nlay+1)  ! in-cloud liquid water path [mcica]
      real(kind=rb) :: cldfmc(nlay+1)  ! cloud fraction [mcica]
      real(kind=rb) :: ciwpmc(nlay+1)  ! in-cloud ice water path [mcica]
      real(kind=rb) :: clwpmc(nlay+1)  ! in-cloud liquid water path [mcica]
...
! Combine gaseous and aerosol optical depths, if aerosol active
         ...
         elseif (iaer .eq. 10) then
		  ...
		  !print*,"taut=",k,ig,taug(k,ig) , taua(k,ngb(ig))
               enddo
            enddo
         endif
...
!  Vertical indexing goes from bottom to top; reverse here for GCM if necessary.
         ...
         enddo
         do k = 0, nlayers-2 !srf - change from "nlayers-1" to "nlauers-2"
	                     !      to acomodate the extra layer
            hr(iplon,k+1) = htr(k)
            hrc(iplon,k+1) = htrc(k)
	    !print*,"in hr/hrc=",iplon,k,real(hr(iplon,k+1),4);call flush(6)
         enddo
...
!  If idrv=1 option is active, transfer upward flux derivatives to output arrays.
         ...
         if (idrv .eq. 1) then 
	       ...
	       !print*,"in lw=",k,duflx_dt(iplon,k+1),duflxc_dt(iplon,k+1);call flush(6)
            enddo
         endif
...
!  from GCM fields should be reversed here if necessary.
      ...
      pz(0) = plev(iplon,1)
      tz(0) = tlev(iplon,1)
      !print*,"zero=",pz(0),tz(0);call flush(6)
      do l = 1, nlayers-1
         pavel(l) = play(iplon,l)
         tavel(l) = tlay(iplon,l)
         pz(l) = plev(iplon,l+1)
         tz(l) = tlev(iplon,l+1)
	 !print*,"pt=",l,real(pavel(l),4),real(pz(l),4),real(tavel(l),4),real(tz(l),4)
!
! For h2o input in vmr:
         wkl(1,l) = h2ovmr(iplon,l)
! For h2o input in mmr: (mass mixing ratio)
...
!
	 ...
	 !print*,"wk1=",l,real(coldry(l),4)!,real(wkl(1:7,l),4)  ;call flush(6)
	 !print*,"o3=",l,real(pavel(l),4),real(wkl(3,l),4)*1.e+9*48./29.&
	 !  ,real(h2ovmr(iplon,l),4)*1000.*18./29.  ;call flush(6)
      enddo

!WRITE (*,fmt='("wkl-1: ",32(E13.3,1X))') (wkl(1,l),l=1,nlayers); call flush(6)
!WRITE (*,fmt='("cld-1: ",32(E13.3,1X))') (coldry(l),l=1,nlayers); call flush(6)
...
! Set cross section molecule amounts from input; convert to vmr if necessary
	 ...
	 !print*,"wx=",l,wx(1:4,l);call flush(6)
      enddo      
...
! water vapor for diffusivity angle adjustments in rtrn and rtrnmr.
      ...
      do l = 1, nlayers
         ...
         enddo
	 !print*,"wk2=",l,real(wkl(1:7,l),4),real(coldry(l),4)  ;call flush(6)
...
!WRITE (*,fmt='("wkl-2: ",32(E13.3,1X))') (wkl(1,l),l=1,nlayers); call flush(6)
...
! modify to reverse layer indexing here if necessary.
     ...
     if (iaer .ge. 1) then
        do l = 1, nlayers-1
           do ib = 1, nbndlw
!              print *,'tauaer: ',iplon,l,ib,tauaer(iplon,l,ib); call flush(6)
...
!         enddo
         ...
         do l = 1, nlayers-1
           ...
           cldfmc(l) = cldfmcl(iplon,l)
           ciwpmc(l) = ciwpmcl(iplon,l)
           clwpmc(l) = clwpmcl(iplon,l)
	   reicmc(l) = reicmcl(iplon,l)
           relqmc(l) = relqmcl(iplon,l)
...
!-srf uncommented for extra layer:
	 ...
	 taucmc(1:ngptlw,nlayers) = 0.0_rb
	 taua  (nlayers,1:nbndlw) = 0.0_rb
