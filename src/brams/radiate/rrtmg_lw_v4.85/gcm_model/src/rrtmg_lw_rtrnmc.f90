...
!         enddo
         ...
         do ig = 1, ngptlw
            if (cldfmc(lay) .eq. 1._rb) then
               ib = ngb(ig)
               odcld(lay,ig) = secdiff(ib) * taucmc(ig,lay)
               transcld = exp(-odcld(lay,ig))
               abscld(lay,ig) = 1._rb - transcld
               efclfrac(lay,ig) = abscld(lay,ig) * cldfmc(lay)
               icldlyr(lay) = 1
            else
               odcld(lay,ig) = 0.0_rb
               abscld(lay,ig) = 0.0_rb
...
!                print *,'LFR-tblind: ',secdiff(iband),taut(lev,igc);call flush(6)
...
!                         d_radlu_dt * (1.0_rb - cldfmc(igc,lev)) * (1.0_rb - atrans(lev))
                  ...
                  d_radlu_dt = d_radlu_dt * cldfmc(lev) * (1.0_rb - atot(lev)) + &
                         d_radlu_dt * (1.0_rb - cldfmc(lev)) * (1.0_rb - atrans(lev))
...
! Set heating rate to zero in top layer
      ...
      return
! print output -----
print*,"BEGIN of rtrnmc";call flush(6)
     print*,"1", totuflux(0:)	  ! upward longwave flux (w/m2)
     			  !    Dimensions: (0:nlayers)
     print*,"2", totdflux(0:)	  ! downward longwave flux (w/m2)
     			  !    Dimensions: (0:nlayers)
     print*,"3", fnet(0:)	  ! net longwave flux (w/m2)
     			  !    Dimensions: (0:nlayers)
     print*,"4", htr(0:) 	  ! longwave heating rate (k/day)
     			  !    Dimensions: (0:nlayers)
     print*,"5", totuclfl(0:)	  ! clear sky upward longwave flux (w/m2)
     			  !    Dimensions: (0:nlayers)
     print*,"6", totdclfl(0:)	  ! clear sky downward longwave flux (w/m2)
     			  !    Dimensions: (0:nlayers)
     print*,"7", fnetc(0:)	  ! clear sky net longwave flux (w/m2)
     			  !    Dimensions: (0:nlayers)
     print*,"8", htrc(0:)	  ! clear sky longwave heating rate (k/day)
     			  !    Dimensions: (0:nlayers)
     print*,"9", dtotuflux_dt(0:)  ! change in upward longwave flux (w/m2/k)
     			  ! with respect to surface temperature
     			  !    Dimensions: (0:nlayers)
     print*,"10", dtotuclfl_dt(0:)  ! change in upward longwave flux (w/m2/k)
     			  ! with respect to surface temperature
print*,"END of rtrnmc";call flush(6)
