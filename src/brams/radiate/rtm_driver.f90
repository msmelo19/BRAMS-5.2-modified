...
contains
    ...
    subroutine rrtm_driver(mzp, mxp, myp, ia, iz, ja, jz, mynum)
        ...
        !
        real,dimension(mzp,mxp,myp) :: lwl,ice_frac,iwl,cloud_fraction
        ...
        !srf -03/03/2013 cloud fraction for operational runs only
...
! ****************************************************************************
   ...
   real, intent(in), dimension(mzp,mxp,myp) :: ice_frac !total cloud ice water 
   ...
   real(kind=rb) :: ssac(nbndsw,mzp-1)
   real(kind=rb) :: asmc(nbndsw,mzp-1)
   real(kind=rb) :: fsfc(nbndsw,mzp-1)
   
   real(kind=rb) :: taucloud_sw(mzp-1,jpband)
   real(kind=rb) :: taucldorig(mzp-1,jpband)
   real(kind=rb) :: ssacloud(mzp-1,jpband) 
   real(kind=rb) :: asmcloud(mzp-1,jpband)
...
!kml                                                      !       duflx_dt and duflxc_dt output   
!   real(kind=rb), allocatable :: cldfmcl_sw(:,:,:)     ! cloud fraction
                                                   !    dimensions: (ngptsw,ncol,nlay)
!   real(kind=rb), allocatable :: cldfmcl_lw(:,:,:)     ! cloud fraction
...
!kml
   ...
   real(kind=rb), allocatable :: taucmcl_lw(:,:,:) ! in-cloud optical depth
                                                   !    dimensions: (ngptsw,ncol,nlay)
   real(kind=rb), allocatable :: ssacmcl(:,:,:)    ! in-cloud single scattering albedo
                                                   !    dimensions: (ngptsw,ncol,nlay)
   real(kind=rb), allocatable :: asmcmcl(:,:,:)    ! in-cloud asymmetry parameter
                                                   !    dimensions: (ngptsw,ncol,nlay)
   real(kind=rb), allocatable :: fsfcmcl(:,:,:)    ! in-cloud forward scattering fraction
                                                   !    dimensions: (ngptsw,ncol,nlay)
!kml
...
!   real(kind=rb), allocatable :: clwpmcl_lw(:,:,:) ! in-cloud liquid water path (g/m2)
   ...
   real(kind=rb), allocatable :: reicmcl(:,:)      ! cloud ice effective radius (microns)
                                                   !    dimensions: (ncol,nlay)
                                                   ...
                                                   !               [dge = 1.0315 * r_ec]
   real(kind=rb), allocatable :: relqmcl(:,:)      ! cloud water drop effective radius (microns)
                                                   !    dimensions: (ncol,nlay)
   real(kind=rb), allocatable :: tauaer(:,:,:)     ! aerosol optical depth (iaer=10 only)
                                                   !    dimensions: (ncol,nlay,nbndsw)
                                                   ! (non-delta scaled)      
   real(kind=rb), allocatable :: tauaer_lw(:,:,:)  ! aerosol optical depth (iaer=10 only)
                                                   !    dimensions: (ncol,nlay,nbndsw)
                                                   ! (non-delta scaled)      
   real(kind=rb), allocatable :: ssaaer(:,:,:)     ! aerosol single scattering albedo (iaer=10 only)
                                                   !    dimensions: (ncol,nlay,nbndsw)
                                                   ! (non-delta scaled)      
   real(kind=rb), allocatable :: asmaer(:,:,:)     ! aerosol asymmetry parameter (iaer=10 only)
                                                   !    dimensions: (ncol,nlay,nbndsw)
                                                   ! (non-delta scaled)      
   real(kind=rb), allocatable :: ecaer(:,:,:)      ! aerosol optical depth at 0.55 micron (iaer=6 only)
...
!   allocate(cldfmcl_sw(ngptsw,nofcols,nlay)) ;cldfmcl_sw=0.0   
   allocate(cldfmcl_lw(nofcols,nlay)) ;cldfmcl_lw=0.0
   allocate(cldfmcl_sw(nofcols,nlay)) ;cldfmcl_sw=0.0   
!kml
   allocate(taucmcl_sw(ngptsw,nofcols,nlay)) ;taucmcl_sw=0.0
   allocate(taucmcl_lw(ngptlw,nofcols,nlay)) ;taucmcl_lw=0.0
   allocate(ssacmcl   (ngptsw,nofcols,nlay)) ;ssacmcl   =0.0
   allocate(asmcmcl   (ngptsw,nofcols,nlay)) ;asmcmcl   =0.0
   allocate(fsfcmcl   (ngptsw,nofcols,nlay)) ;fsfcmcl   =0.0
!kml
...
!   allocate(clwpmcl_lw(ngptlw,nofcols,nlay)) ;clwpmcl_lw=0.0
   allocate(ciwpmcl_sw(nofcols,nlay)) ;ciwpmcl_sw=0.0
   allocate(clwpmcl_sw(nofcols,nlay)) ;clwpmcl_sw=0.0
   allocate(ciwpmcl_lw(nofcols,nlay)) ;ciwpmcl_lw=0.0
   allocate(clwpmcl_lw(nofcols,nlay)) ;clwpmcl_lw=0.0


   allocate(reicmcl   (nofcols,nlay))        ;reicmcl   =0.0
   allocate(relqmcl   (nofcols,nlay))        ;relqmcl   =0.0
   ...
   allocate(tauaer    (nofcols,nlay,nbndsw)) ;tauaer    =0.0
   allocate(tauaer_lw (nofcols,nlay,nbndlw)) ;tauaer_lw =0.0
   allocate(ssaaer    (nofcols,nlay,nbndsw)) ;ssaaer    =0.0
   allocate(asmaer    (nofcols,nlay,nbndsw)) ;asmaer    =0.0
   ...
   do j=ja,jz
      do i=ia,iz
         ...
         do k=2,mzp
            ...
            cldfmcl_sw(noc,k-1)=cloud_fraction(k,i,j) !cloud_fraction
            cldfmcl_lw(noc,k-1)=cloud_fraction(k,i,j)
            clwpmcl_sw(noc,k-1)=lwl(k,i,j) !total cloud liquid water (kg/kg for carma and g/m2 for rrtm)
            clwpmcl_lw(noc,k-1)=lwl(k,i,j) 
            ciwpmcl_sw(noc,k-1)=iwl(k,i,j) !total cloud ice water (kg/kg for carma and g/m2 for rrtm)
            ciwpmcl_lw(noc,k-1)=iwl(k,i,j)

            relqmcl(noc,k-1)=micro_g(ngrid)%rel(k,i,j) ! effective radius liquid
            reicmcl(noc,k-1)=micro_g(ngrid)%rei(k,i,j) ! effective radius ice
	 ...
	 plev(noc,mzp)=plev(noc,mzp-1)-2.*(plev(noc,mzp-1)-play(noc,nlay))
         ...
         if(firsttime .or. chemistry < 0) then 
            ...
            do k=2,mzp
               do np=1,nbndsw
                  tauaer(noc,k-1,np)=(carma_tuv(ngrid)%g_tauaer(bndmin_sw(np),k,i,j) + &!matrix desligado
                                      carma_tuv(ngrid)%g_tauaer(bndmax_sw(np),k,i,j)) * 0.5
                  aot_rrtm_sw(i,j,np) = aot_rrtm_sw(i,j,np)+tauaer(noc,k-1,np)
                  ssaaer(noc,k-1,np)=(carma_tuv(ngrid)%g_wol   (bndmin_sw(np),k,i,j) + &
                                      carma_tuv(ngrid)%g_wol   (bndmax_sw(np),k,i,j)) *0.5
                  asmaer(noc,k-1,np)=(carma_tuv(ngrid)%g_gol   (bndmin_sw(np),k,i,j) + &
                                      carma_tuv(ngrid)%g_gol   (bndmax_sw(np),k,i,j)) *0.5
               end do
            end do
         end if
        ...
        if(firsttime .or. chemistry < 0) then 
            ...
            do k=2,mzp
               do np=1,nbndlw
                  tauaer_lw(noc,k-1,np)=(carma_tuv(ngrid)%g_tauaer(bndmin_lw(np),k,i,j) + &!matrix desligado
                                         carma_tuv(ngrid)%g_tauaer(bndmax_lw(np),k,i,j)) * 0.5
...
!srf         sig(i)=plev(2,mzp-i+1)/ plev(2,1) !plev(2,mzp)
         sig(i)=plev(2,i)/ plev(2,1) !plev(2,mzp)
      end do
      sig(1)=1.
      sig(mzp+1)=0.
    ...
    doy=real(dyofyr,r8)
    call getoz_rrtm (nofcols,nofcols,mzp,sigmid,colrad,doy,o3l)
...
!-----tmp
   ...
   !- longwave section
   do noc=1,nofcols
      call cldprop(nlay, inflglw, iceflglw, liqflglw, cldfmcl_lw(noc,:), tauc_lw, &
                         ciwpmcl_lw(noc,:), clwpmcl_lw(noc,:), &
                         reicmcl(noc,:), relqmcl(noc,:), ncbandslw, taucloud_lw)
      do np=1,nbndlw
         taucmcl_lw(np,noc,:)=taucloud_lw(:,np) 
      end do
   end do
...
!----------------------------------
   ...
   do noc=1,nofcols
          call cldprop_sw(nlay, inflgsw, iceflgsw, liqflgsw, cldfmcl_sw(noc,:) , &
                            ...
                            clwpmcl_sw(noc,:),reicmcl(noc,:), relqmcl(noc,:), &
                            taucldorig, taucloud_sw, ssacloud, asmcloud)
      do np=1,nbndsw

         taucmcl_sw(np,noc,:)= taucloud_sw(:,np+15)
         ssacmcl   (np,noc,:)= ssacloud   (:,np+15)
         asmcmcl   (np,noc,:)= asmcloud   (:,np+15)
   ...
   if(mynum== 1) then
    ...
    write(mynum+5,*) "mynum=",mynum
    write(mynum+5,*) "max/min  swhr ",maxval(swhr),minval(swhr)
    write(mynum+5,*) "max/min  hr   ",maxval( hr ),minval( hr)
    write(mynum+5,*) "max/min rlong ",maxval(radiate_g(ngrid)%rlong),minval(radiate_g(ngrid)%rlong)
    write(mynum+5,*) "max/min rshort",maxval(radiate_g(ngrid)%rshort),minval(radiate_g(ngrid)%rshort)
    write(mynum+5,*) "max/min fthrd ",maxval(86400.*radiate_g(ngrid)%fthrd ),&
                 		      minval(86400.*radiate_g(ngrid)%fthrd )
    write(mynum+5,*) "max/min theta ",maxval(basic_g(ngrid)%theta),minval(basic_g(ngrid)%theta)
    write(mynum+5,*) "============= radiation-rrtm ===================="
    call flush(mynum+5)
   endif

   !call map2d(co3fromtuv(20,:,:),'./gpl/o3',20,int(time),mxp,myp,.true.,.false.)
   !call map2d(radiate_g(ngrid)%rshort,'./gpl/rshort',1,int(time),mxp,myp,.true.,.false.)
        ...
        if (igraup>0) then
          if(mcphys_type <= 1) then
            do k=1,m1
              ...
              enddo
            enddo
          elseif(mcphys_type == 2 .or. mcphys_type ==3) then  !srf -gthompson microphysics - graupel only in ice phase
            dummy_vec=0.0
            endif
...
!srf -tmp ---------------------
 ...
 subroutine InitGetoz_rrtm(yrl,kmax,sl)
  ...
  SUBROUTINE getoz_rrtm (ncols,adjNCols, kmax,sigmid,colrad,date,o3l)
    !
