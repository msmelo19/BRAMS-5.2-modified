...
!--(DMK-CCATT-FIM)-----------------------------------------------------------
  ...
  INTERFACE
     ...
     subroutine sfclmcv(ustar, tstar, rstar, vels, vels_pat, ups, vps, &
...
! Local Variables:
  ...
  ! For TEB
  IF (TEB_SPM==1 .and. present(G_URBAN)) then
     if (NINT(G_URBAN)/=0) goto 100
  endif
...
100 continue ! For TEB is necessary to jump the lines above
