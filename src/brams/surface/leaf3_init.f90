...
use leaf_coms, only: &
     ...
     dead_frac, rcmin, glai_max, root, kroot
...
, .00,    .00   & !  3  Bare ground
,2.43,   4.40   & !  4  Evergreen needleleaf tree
,3.46,   4.20   & !  5  Deciduous needleleaf tree
,3.98,   4.10   & !  6  Deciduous broadleaf tree
,0.50,   4.90   & !  7  Evergreen broadleaf tree (orig a=1.87 )
,1.00,   0.15   & !  8  Short grass  (orig= 3.97)
,2.84,   1.40   & !  9  Tall grass
,2.53,   0.40	& ! 10  Semi-desert
,8.99,   1.20   & ! 11  Tundra
,3.67,   4.80   & ! 12  Evergreen shrub
,3.67,   4.80   & ! 13  Deciduous shrub
,2.84,   1.40   & ! 14  Mixed woodland
,2.00,   0.15   & ! 15  Crop/mixed farming(orig= 3.97)
,3.97,   0.15   & ! 16  Irrigated crop
,3.97,   0.15   & ! 17  Bog or marsh
  ...
  do nnn = 1,nvtyp
   ...
   if(nnn > 3) then
    ...
    !print*,'veg=',nnn
        sum=0.
   	do k = nzg,1,-1
	   root(nnn,k)=1.-exp(max(-30.,bp*slz(k)/B_alfa))-sum
	   sum = sum + root(nnn,k)
	   !print*,k,slz(k),root(nnn,k),sum
           if(k==1) then
	      if(sum < 1.) then
	       sum=1.-sum
	       root(nnn,k) = sum + root(nnn,k)
	      endif
	   endif
!	   print*,k,slz(k-1),root(nnn,k),sum
!	   print*,root(2,nnn)+root(3,nnn)+root(4,nnn)
	enddo
	sum = 0.
   	do k = nzg,1,-1
	   sum = sum + root(nnn,k)
	enddo
	if(abs(sum-1.) .gt. 1.e-6) then
	 print*,'root-1 > 1.e-6', sum,sum-1.
	 stop 43443
	endif
  endif
 enddo
