   ...
   do j = 1,n3
      do i = 1,n2
         do ipat = 2,npat
            ...
            if (nint(leaf_class(i,j,ipat)) == 10 .or. nint(leaf_class(i,j,ipat)) ==3 ) then  !   semi-deserto
...
return
   do j = 1,n3
      do i = 1,n2
	if( (i>39 .and. i< 60) .and. (j>13 .and. j < 31))then ! .and. &
	    !(nint(leaf_class(i,j,2)) == 18) )                    then
	     leaf_class(i,j,2)=8
	else
	     leaf_class(i,j,2)=7
	endif 
	print*,"veg=",i,j,leaf_class(i,j,2)
      enddo
   enddo
! if (ifm .eq. 1) then
...
subroutine change_soil_moisture_init(n1,n2,n3,mzg,mzs,npat,ifm   &
     ,theta,pi0,pp  &
     ,soil_water     ,soil_energy      ,soil_text          &
     ,sfcwater_mass  ,sfcwater_energy  ,sfcwater_depth     &
     ,glat,glon  &
  ...
  real, dimension(mzs,n2,n3,npat) :: sfcwater_mass,sfcwater_energy  &
       ,sfcwater_depth
  real, dimension(n2,n3,npat) :: leaf_class
  ...
  !- the changes in the soil_moisture field must be customized by the user.
  !return 
  if(mynum==2)print*,"======> changing the initial soil moisture - check ruser.f90 routine"
     ...
     do i = 1,n2
        ...
        do ipat= 2,npat
          ...
          do k = 1,mzg
           ...
           nsoil = nint(soil_text(k,i,j,ipat))
           fx(:)=1.
           if(nint(leaf_class(i,j,2)) == 7) then  !tropical forest
            fx(:)=1.5
           else
            fx(:)=1.2
           endif
   
!             if(nint(leaf_class(i,j,2)) == 18) then
!	          soil_text(k,i,j,ipat) = 3.
!		  nsoil = nint(soil_text(k,i,j,ipat))
!                 soil_water(k,i,j,ipat) = 1.3*soilcp(nsoil)
!              endif

!- change to "fx" of the original value:
              !print*,"sm1=",soil_water(k,i,j,ipat),fx(k),fx(k)*soil_water(k,i,j,ipat)
              soil_water(k,i,j,ipat) = fx(k)*soil_water(k,i,j,ipat)

              soil_water(k,i,j,ipat) = max(soilcp(nsoil), &
                                       min(soil_water(k,i,j,ipat), slmsts(nsoil)))

              !print*,"sm2=",soil_water(k,i,j,ipat)
              soil_energy(k,i,j,ipat) = (airtemp - 273.15 + stgoff(k))   &
                   * (slcpd(nsoil) + soil_water(k,i,j,ipat)  * 4.186e6)  &
                   + soil_water(k,i,j,ipat)  * 3.34e8
           enddo
  ...
  enddo
  return
end subroutine change_soil_moisture_init
!--(DMK-CCATT-FIM)-----------------------------------------------------------
