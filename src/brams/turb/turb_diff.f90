...
!!$  if (idiffkk .ge. 3) then
  ...
  !write (*,fmt='("turb1: ",I2.2,1X,I1.1,1X,4(E9.3,1X),"/",4(E9.3,1X))') mynum,1,(wt(k,2,2),k=1,4),(wt(k,3,3),k=1,4)
  !call flush(6)
  ...
  !write (*,fmt='("turb2: ",I2.2,1X,I1.1,1X,4(E9.3,1X),"/",4(E9.3,1X))') mynum,1,(wt(k,2,2),k=1,4),(wt(k,3,3),k=1,4)
  !call flush(6)
...
!!$  integer, intent(IN) :: nsc
  ...
  if (n==1 .or. ksf/=ksf_save) then
     ...
     do j=ja,jz
        do i=ia,iz
           c1(i,j) = 0.5*dtlt/(rtgt(i,j)*rtgt(i,j))
        enddo
     enddo
     ! **(JP)** fim de modificacao
  ...
  if (ihorgrad==1) then
     ...
     do j=ja,jz
        do i=1,iz
           do k=1,m1-1
              vt3df(k,i,j) = -vt3df(k,i,j)*0.5*(hkkh(k,i,j) + hkkh(k,i+1,j))
           enddo
        enddo
     enddo
     ...
     do j=1,jz
        do i=ia,iz
           do k=1,m1-1
              vt3dg(k,i,j) = -vt3dg(k,i,j)*0.5*(hkkh(k,i,j) + hkkh(k,i,j+jd))
           enddo
        enddo
     enddo
  endif
...
subroutine PBLforcing(ngrid, m1, m2, m3, ia, iz, ja, jz, vt3df, scp, lsfcupar, nsc)
  ...
  if (nnqparm(ngrid)>=2) then
     ...
     if ( scalar_tab(nsc,ngrid)%name=='THP' .or. &
        ...
        do j=ja,jz
           do i=ia,iz
              do k=2,m1-1
		 ! Forcing with only the vertical diffusion
                 lsfcupar(k,i,j) = &
