...
!--- Arguments:---------------------------------------------------------
SUBROUTINE phi_m_h_vol (                                          &
 row_length,rows,points,tile_pts,tile_index,pts_index,            &
...
IF (lhook) CALL dr_hook('PHI_M_H_VOL',zhook_in,zhook_handle)

IF (ltimer) THEN
! DEPENDS ON: timer
  CALL timer('PHI_M_H_VOL ',3)
END IF
...
IF (ltimer) THEN
! DEPENDS ON: timer
  CALL timer('PHI_M_H_VOL ',4)
END IF

IF (lhook) CALL dr_hook('PHI_M_H_VOL',zhook_out,zhook_handle)
RETURN
